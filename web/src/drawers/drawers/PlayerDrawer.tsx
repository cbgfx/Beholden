...
export function PlayerDrawer(props: {
  ...
  React.useEffect(() => {
    ...
    setForm({
      ...
      ac: String(p.ac),
      lvl: String(p.level),
      speed: String((p as any).speed ?? 30),
      pStr: String((p as any).str ?? 10),
      pDex: String((p as any).dex ?? 10),
      pCon: String((p as any).con ?? 10),
      pInt: String((p as any).int ?? 10),
      pWis: String((p as any).wis ?? 10),
      pCha: String((p as any).cha ?? 10)
    });
