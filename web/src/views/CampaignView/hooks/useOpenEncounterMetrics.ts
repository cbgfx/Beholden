...
export function useOpenEncounterMetrics(args: {
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      ...
      const partyHpMax = (players ?? []).reduce(
      ...
      for (const encId of toFetch) {
        try {
          ...
          const missing = Array.from(monsterIds).filter((id) => !(monsterDetails && (monsterDetails as any)[id]));
          ...
          const details = { ...(monsterDetails as any), ...patch };
  ...
  }, [dispatch, encounterDifficulty, encounterXp, encounters, inpcs, monsterDetails, players]);
