...
export function useMonsterPickerRows(args: {
  ...
  const preparedRows = React.useMemo<PreparedMonsterRow[]>(() => {
    return args.rows.map((m) => {
      ...
      const envRaw = String((m as any)?.environment ?? "").trim();
      ...
      return {
        ...
        crNum: parseCrNumber((m as any).cr),
        firstLetter
      };
    });
  }, [args.rows]);
  ...
  const filteredRows = React.useMemo(() => {
    ...
    return baseSorted.filter((m) => {
      ...
      if (hasCr) {
        ...
        if (Number.isFinite(min) && v < (min as number)) return false;
        if (Number.isFinite(max) && v > (max as number)) return false;
      }
